...
# this scrit will download the .tar.ba2 file from waterfox chosen url;
# extract it to /usr/lib64/;
# create the .desktop file
# create a symbolic link in /usr/bin pointing to /usr/lib64/waterfox/waterfox
#
# As there is not yeat a waterfox-latest.tar.bz2 the url has to be pasted here for each new version
...
echo -e "Downloading...\n"
wget -t 5 -T 10 -w 10 --waitretry=15 -c $WFXURL
...
#waterfox.desktop taken from AUR : waterfox-bin.git https://aur.archlinux.org/cgit/aur.git/plain/waterfox.desktop?h=waterfox-bin
tee -i /usr/share/applications/waterfox.desktop <<WFOX
[Desktop Entry]
Name=Waterfox
...
ln -s /usr/lib64/waterfox/waterfox /usr/bin/waterfox
else
